...
python3 "$DIR/extract.py" "$HTML" "$TMP/app.js" \
  ...
  bpInvLot bpOrderById bpOrderCifLb bpOrderPerCaseLb bpInvRate bpDaysSince bpEtaOf invmCanonSku bpArrivalNetCases bpGetPipelineByWeek bpRenderFifo dmcAdd ordAfterBoughtForChange ordMergeLocalOnly _mtoMarkList _mtoMarksMap getOrders saveOrders renderCommittedPanel simGingerPeruOrders simRenderProduct bpShowAvailableFree dmForwardWeeks dmMondayISO invmAnalysisHTML invmRenderProduct prodCommittedTotal renderBuyPlanner renderDemandAccuracy renderDmRunRate simRenderProjection isoWeek normalizeDate _cmCrossDock renderHome invmProductModel invmProductModel
