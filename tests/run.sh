...
python3 "$DIR/extract.py" "$HTML" "$TMP/app.js" \
  ...
  invmProductStats invmCommittedByWeek cmFirmForWeek cmWeekBadge cmWeekOrdersHTML bpDateWk invmWeekDemands invmWinModel invmProductArrivals bpProtectPlan bpContainerPlan bpOrderSchedule bpRowAtDate invmOrderCases invmDeliveryCalendars invmPlanDeliveries bpGingerSuggestionHTML invmRunRateLbs _invmKnownOrigin invmOriginsFor invmOriginMatch dsWindow invmStdev invmZFromService productCaseLb mtoNetRows mtoByCustomer mtoCasesPerWeek dmcArrivingOrders ordBoughtForCustomers addDirectShip removeDirectShip directShipTotal mtoEarmarkedTotal getDirectShip _mutateOrderDirectShip bpInvState bpInvMigrateV1 invmProjectionHTML invmDirectShipOrdersHTML invmMtoCandidatesHTML _dsDesde invmOverview _sheetSafe _sheetSafeRows _ordSanitize dmcNormalizeUnshipped dmcExcelDate cmCasesInBuyPack cmPlanEntries dmWeekPace _dmCurveMemo _dmWeekDaySeries dmWeekCurve dmWeekShareBefore dmWeekShareReport bpSnapWeekDemand renderWeekPanel dmWeekOrdersOpen dmWeekToggleOrders hybridSalesForWeek dsNamesFor dsLabelFor jlzSyncScope getActiveOrigin \
  ...
  invmReviewOld invmReviewItems invmReviewTally invmReviewCount invmReviewCounts invmReviewBadgeHTML invmReviewPillHTML invmReviewHintHTML _invmRevKey invmReviewFilterOf invmSetReviewFilter invmReviewBoxHTML invmReviewReport
...
for _c in DM_SELL_DAYS _invmWinModels DM_CURVE_WKS BP_INV_LS BP_INV_LS_V1 BP_PE_CALENDAR CODY_PRODUCTS WLOT_SNAP_LS WLOT_TONE WLOT_TONE_TITLE WLOT_FILTER _invmOrigDemandCache MKT_LS MKT_RULE_NOTE _mktCtx INVM_REVIEW_DAYS INVM_REVIEW_FILTER INVM_REVIEW_CLOCK; do
