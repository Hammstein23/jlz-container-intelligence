...
(function(){
  ...
  var editada = mktApply(r.map, 'turmeric', 'Hawaii', 'edit', { from:'2026-07-27', back:'' }, T);
  check('la fecha de inicio se puede corregir hacia atrás', editada.map['turmeric|Hawaii'][0].from, '2026-07-27');
  check('y la vuelta esperada también', editada.map['turmeric|Hawaii'][0].back, '');
  ok('corregir no la reactiva', editada.map['turmeric|Hawaii'][0].to === '' && !!mktPauseActive('turmeric', 'Hawaii', T, editada.map));
  ok('no se corrige a una fecha futura', !!mktApply(r.map, 'turmeric', 'Hawaii', 'edit', { from:'2026-09-20' }, T).error);
  ok('sin marca abierta no hay qué corregir', !!mktApply({}, 'turmeric', 'Hawaii', 'edit', { from:'2026-08-10' }, T).error);
...
(function(){
  ...
  ok('la fecha se puede corregir desde la barra', bar.indexOf('Change dates') > -1);
  var vacia = mktPausedCardHTML({ from:'2026-09-15' }, 0, 0, 0);
  ok('sin stock dice que no hay, no "0.0 weeks of cover from 0 cases"', vacia.indexOf('no stock on hand') > -1 && vacia.indexOf('0.0 weeks') < 0);
  ok('con stock dice cuánto dura', mktPausedCardHTML({ from:'2026-09-01' }, 2.5, 50, 0).indexOf('2.5 weeks of cover from 50 cases') > -1);
